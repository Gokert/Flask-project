...
def vacancy_add():
    ...
    position = select_dict(current_app.config['db_config'], provider.get('find_positions.sql', Name_pos=head))
    if position:
        insert(current_app.config['db_config'],
               provider.get("insert_vacancy.sql", Name=head, txt=text, Open_date=str(dt_now).split()[0]))
